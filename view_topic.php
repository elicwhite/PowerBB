...
else if ($action == 'new' && !$forum_user['is_guest'])
{
	...
	if ($first_new_post_id) header('Location: view_topic.php?pid='.$first_new_post_id.'#p'.$first_new_post_id);
	else header('Location: view_topic.php?id='.$id.'&action=last');
	exit;
}
else if ($action == 'last')
{
...
{
	if (($cur_topic['post_replies'] == '' && $forum_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1' || $is_admmod) $post_link = '<a href="post.php?tid='.$id.'">'.$lang_topic['Post reply'].'</a>';
	else $post_link = '<a href="login.php">'.$lang_topic['Post reply'].'</a>'; 
}
else
{
	$post_link = $lang_topic['Topic closed'];
	if ($is_admmod) $post_link .= ' / <a href="post.php?tid='.$id.'">'.$lang_topic['Post reply'].'</a>';
}
$num_pages = ceil(($cur_topic['num_replies'] + 1) / $forum_user['disp_posts']);
$p = (!isset($_GET['p']) || $_GET['p'] <= 1 || $_GET['p'] > $num_pages) ? 1 : $_GET['p'];
$start_from = $forum_user['disp_posts'] * ($p - 1);
$paging_links = $lang_common['Pages'].': '.paginate($num_pages, $p, 'view_topic.php?id='.$id);
...
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($cur_post['poster_id'] > 1)
	{
		...
		if($cur_post['displayname'] != "")
			{
				$cur_post['username'] = $cur_post['displayname'];
			}		
...
//if ($cur_post['yahoo'] != '' && !$forum_user['is_guest']) $user_contacts[] = '<a href="ymsgr:sendim?'.$cur_post['yahoo'].'"><img src="http://opi.yahoo.com/online?u='.$cur_post['yahoo'].'&m=g&t=3" border=0 alt="YM" title="Send '.$cur_post['username'].' instant message!"></a>'; 
	...
	if (!$is_admmod)
	{
		...
		if ($cur_topic['closed'] == '0')
		{
			...
			if (($cur_topic['post_replies'] == '' && $forum_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1') $post_actions[] = '<li class="postquote"><a href="post.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a>';
		}
	}
	else $post_actions[] = '<li class="postreport"><a href="misc.php?report='.$cur_post['id'].'">'.$lang_topic['Report'].'</a>'.$lang_topic['Link separator'].'</li><li class="postdelete"><a href="delete.php?id='.$cur_post['id'].'">'.$lang_topic['Delete'].'</a>'.$lang_topic['Link separator'].'</li><li class="postedit"><a href="edit.php?id='.$cur_post['id'].'">'.$lang_topic['Edit'].'</a>'.$lang_topic['Link separator'].'</li><li class="postquote"><a href="post.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a>';
...
?> : 
<?php 
            	if($forum_user['is_guest'] != true && $forum_user['username'] != $cur_post['displayname'] || $cur_post['username'])
			{ 
?>
...
?>
			&nbsp;<strong><?php echo $cur_post['reputation_plus'] + $cur_post['reputation_minus']; ?></strong>
<?php
		}
?>
		</dd>
